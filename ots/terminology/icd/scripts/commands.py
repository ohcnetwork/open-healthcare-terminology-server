...
def download(ctx: typer.Context) -> None:
    ...
    raise typer.Exit(run_module_main("ots.terminology.icd.scripts.download_releases", ctx.args))


@app.command("load-10cm", context_settings=CONTEXT_SETTINGS)
def load_10cm(ctx: typer.Context) -> None:
    """Load ICD-10-CM data."""
    raise typer.Exit(run_module_main("ots.terminology.icd.scripts.load_icd10cm_postgres", ctx.args))


@app.command("load-11", context_settings=CONTEXT_SETTINGS)
def load_11(ctx: typer.Context) -> None:
    """Load ICD-11 MMS data."""
    raise typer.Exit(run_module_main("ots.terminology.icd.scripts.load_icd11_postgres", ctx.args))
