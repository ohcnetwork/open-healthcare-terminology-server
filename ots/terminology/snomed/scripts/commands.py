...
def load(ctx: typer.Context) -> None:
    ...
    raise typer.Exit(run_module_main("ots.terminology.snomed.scripts.load_snomed_postgres", ctx.args))
...
def load_packages(ctx: typer.Context) -> None:
    ...
    raise typer.Exit(
        run_module_main("ots.terminology.snomed.scripts.load_snomed_rf2_packages", ctx.args)
    )
...
def rebuild_search_text(ctx: typer.Context) -> None:
    ...
    raise typer.Exit(
        run_module_main("ots.terminology.snomed.scripts.rebuild_snomed_search_text", ctx.args)
    )
